...
const mockDb = {
    ...
    init() {
        ...
        if (!localStorage.getItem(this.KEYS.USUARIOS)) {
            const usuariosIniciais = [
                { id: 1, nome: 'Marcos Souza', email: 'marcos@fabrica.com', matricula: '3001', senha: 'senha123', cargo: 'Almoxarife', status_usuario: 'Ativo' },
                { id: 2, nome: 'Carlos Silva', email: 'carlos@fabrica.com', matricula: '2001', senha: 'senha123', cargo: 'Técnico', status_usuario: 'Ativo', especialidade: 'Elétrica' },
                { id: 3, nome: 'Wanderillo de Castro', email: 'wanderillo@fabrica.com', matricula: '4001', senha: 'senha123', cargo: 'Operador', status_usuario: 'Ativo' },
                { id: 4, nome: 'Admin SGM', email: 'admin@fabrica.com', matricula: '1001', senha: '123456', cargo: 'Administrador', status_usuario: 'Ativo' }
            ];
            localStorage.setItem(this.KEYS.USUARIOS, JSON.stringify(usuariosIniciais));
        }

        // 2. EQUIPAMENTOS
        if (!localStorage.getItem(this.KEYS.EQUIPAMENTOS)) {
            const equipamentosIniciais = [
                { tag: 'MQ-01', nome: 'Máquina de Costura Reta Industrial', setor: 'Costura', critico: false, status_equipamento: 'Operando' },
                { tag: 'MQ-02', nome: 'Prensa Hidráulica de Solado', setor: 'Montagem - Linha Bravo', critico: true, status_equipamento: 'Em Manutenção' },
                { tag: 'MQ-03', nome: 'Chanfradora de Couro', setor: 'Corte', critico: false, status_equipamento: 'Operando' },
                { tag: 'MQ-04', nome: 'Rebitadeira Semiautomática', setor: 'Montagem', critico: false, status_equipamento: 'Operando' },
                { tag: 'MQ-05', nome: 'Esteira de Secagem e Colagem', setor: 'Acabamento', critico: false, status_equipamento: 'Operando' },
                { tag: 'EST-01', nome: 'Esteira Alimentadora Principal', setor: 'Montagem - Linha Bravo', critico: true, status_equipamento: 'Operando' }
            ];
            localStorage.setItem(this.KEYS.EQUIPAMENTOS, JSON.stringify(equipamentosIniciais));
        }
        ...
        if (!localStorage.getItem(this.KEYS.KITS_PADRAO)) {
            const kitsIniciais = [
                { id: 1, nome_kit: 'Kit Padrão Mecânica Avançada #01', maquina_vinculo: 'Prensa MQ-02', ferramentas: 'Jogo Chaves Allen, Chave Inglesa 10", Martelo de Retrocesso', status: 'Disponível' },
                { id: 2, nome_kit: 'Kit Padrão Mecânica Avançada #02', maquina_vinculo: 'Prensa MQ-02', ferramentas: 'Jogo Chaves Allen, Chave Inglesa 10", Alicate de Pressão', status: 'Em uso na OS #102' },
                { id: 3, nome_kit: 'Kit Calibração Elétrica Flash', maquina_vinculo: 'Injetora MQ-09', ferramentas: 'Multímetro Fluke 179, Alicate Amperímetro, Chaves Isoladas', status: 'Disponível' }
            ];
        ...
        if (!localStorage.getItem(this.KEYS.ORDENS_SERVICO)) {
            const osIniciais = [
                {
                    ...
                    tipo_falha: 'Elétrica',
                    descricao_problema: 'A prensa parou no meio do ciclo de compressão. Painel digital piscando em vermelho apresentando erro de subtensão no circuito de potência. Máquina parada totalmente bloqueando a esteira alimentadora.',
                ...
                },
                {
                    ...
                    tipo_falha: 'Mecânica',
                    descricao_problema: 'Ruído excessivo e vibração anormal na esteira de transporte durante o processo de colagem. Suspeita de desalinhamento ou falha no rolamento motor.',
                    solicitante_id: 3,
                    tecnico_id: null,
                    status_os: 'Aberta',
                ...
                },
                {
                    ...
                    tipo_falha: 'Mecânica',
                    descricao_problema: 'Lançadeira travando intermitentemente ao costurar materiais de maior gramatura (couro sintético). Agulha quebrando com frequência.',
                    solicitante_id: 3,
                    tecnico_id: null,
                    status_os: 'Aberta',
            ...
            localStorage.setItem(this.KEYS.ORDENS_SERVICO, JSON.stringify(osIniciais));
        }
        ...
        if (!localStorage.getItem(this.KEYS.REQUISICOES_MATERIAIS)) {
            ...
            localStorage.setItem(this.KEYS.REQUISICOES_MATERIAIS, JSON.stringify(requisicoesIniciais));
        }
        ...
        if (!localStorage.getItem(this.KEYS.CONTROLE_FERRAMENTAL)) {
            ...
            localStorage.setItem(this.KEYS.CONTROLE_FERRAMENTAL, JSON.stringify(cautelasIniciais));
        }
        ...
        if (!localStorage.getItem(this.KEYS.HISTORICO_MAQUINAS)) {
            ...
            localStorage.setItem(this.KEYS.HISTORICO_MAQUINAS, JSON.stringify(historicoInicial));
        }
    ...
    definirUsuarioPadraoPorRota() {
        ...
        if (userPadrao) {
            ...
            console.log(`[SGM DB] Usuário simulado definido automaticamente: ${userPadrao.nome} (${userPadrao.cargo})`);
        }
    },
    ...
    getPathPrefix() {
        const path = window.location.pathname;
        if (path.includes('/Usuario/') || path.includes('/Tecnico/') || path.includes('/Almoxarifado/') || path.includes('/Administrador/')) {
            return '../';
        }
        return './';
    },
    ...
    saveEquipamento(equipamento) {
        ...
        localStorage.setItem(this.KEYS.EQUIPAMENTOS, JSON.stringify(equipamentos));
        return equipamento;
    },
    ...
    saveItemAlmoxarifado(item) {
        ...
        if (!item.codigo) {
            ...
            item.codigo = item.categoria === 'Peça de Reposição' ? `PE-${sufixo}` : `FE-${sufixo}`;
        }
        itens.push(item);
        localStorage.setItem(this.KEYS.ITENS_ALMOXARIFADO, JSON.stringify(itens));
        return item;
    },
    ...
    saveKitPadrao(kit) {
        ...
        kits.push(kit);
        localStorage.setItem(this.KEYS.KITS_PADRAO, JSON.stringify(kits));
        return kit;
    },
    ...
    saveOrdemServico(os) {
        ...
        const proximoNumero = osList.length > 0 ? Math.max(...osList.map(o => {
        ...
        osList.push(os);
        localStorage.setItem(this.KEYS.ORDENS_SERVICO, JSON.stringify(osList));
        return os;
    },
    updateOrdemServico(id, dados) {
        const osList = this.getOrdensServico();
        const index = osList.findIndex(o => o.id === parseInt(id) || o.codigo_os === id);
        if (index !== -1) {
            osList[index] = { ...osList[index], ...dados };
            localStorage.setItem(this.KEYS.ORDENS_SERVICO, JSON.stringify(osList));
            return osList[index];
        }
        return null;
    ...
    saveRequisicao(req) {
        ...
        req.id = requisicoes.length > 0 ? Math.max(...requisicoes.map(r => r.id)) + 1 : 1;
        req.data_solicitacao = new Date().toISOString();
        req.status_requisicao = 'Pendente';
        
        requisicoes.push(req);
        localStorage.setItem(this.KEYS.REQUISICOES_MATERIAIS, JSON.stringify(requisicoes));
        return req;
    },
    updateRequisicao(id, dados) {
        ...
        if (index !== -1) {
            requisicoes[index] = { ...requisicoes[index], ...dados };
            localStorage.setItem(this.KEYS.REQUISICOES_MATERIAIS, JSON.stringify(requisicoes));
    ...
    saveControleFerramental(cf) {
        ...
        cfList.push(cf);
        localStorage.setItem(this.KEYS.CONTROLE_FERRAMENTAL, JSON.stringify(cfList));
        return cf;
    },
    updateControleFerramental(id, dados) {
        const cfList = this.getControleFerramental();
        const index = cfList.findIndex(c => c.id === parseInt(id) || c.codigo_retorno === id);
        if (index !== -1) {
            cfList[index] = { ...cfList[index], ...dados };
            localStorage.setItem(this.KEYS.CONTROLE_FERRAMENTAL, JSON.stringify(cfList));
            return cfList[index];
        }
        return null;
    ...
    addHistoricoLog(tag, log) {
        ...
        historicos[tag].total = count === 1 ? "01 registro" : `${String(count).padStart(2, '0')} registros`;
        
        localStorage.setItem(this.KEYS.HISTORICO_MAQUINAS, JSON.stringify(historicos));
    ...
    injectDeveloperHUD() {
        ...
        if (document.getElementById('view-profiles') || window.location.pathname.endsWith('login.html') || window.location.pathname === '/') {
            return;
        }
        ...
        document.getElementById('sgm-hud-reset').addEventListener('click', () => {
            if (confirm('Deseja realmente limpar todos os dados do LocalStorage e voltar ao padrão do projeto?')) {
                this.reset();
            }
        });
